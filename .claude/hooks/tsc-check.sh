...
# Use project-local cache directory to match stop-build-check-enhanced.sh
...
# Ensure generated artifacts are fresh before type-checking, so we don't report
# false errors from stale CSS-module .d.ts files or a stale shared-types dist build.
# (The project's own `type-check` script regenerates these; bare `tsc` does not.)
prepare_repo() {
    local repo="$1"
    local repo_path="$2"

    if [ "$repo" = "website" ]; then
        # 1) website/tsconfig.json path-aliases @stewra/shared-types -> ../packages/shared-types/dist.
        #    Rebuild that dist when its source is newer (or dist is missing).
        local st="$CLAUDE_PROJECT_DIR/packages/shared-types"
        if [ -d "$st/src" ]; then
            if [ ! -f "$st/dist/index.d.ts" ] || \
               [ -n "$(find "$st/src" -name '*.ts' -newer "$st/dist/index.d.ts" -print -quit 2>/dev/null)" ]; then
                ( cd "$st" && npm run build >/dev/null 2>&1 )
            fi
        fi
        # 2) Regenerate CSS-module type declarations (happy-css-modules), which the
        #    project's `type-check` script runs before tsc.
        ( cd "$repo_path" && npx hcm 'src/**/*.module.css' >/dev/null 2>&1 )
    fi
}
...
run_tsc_check() {
    ...
    # Refresh generated artifacts before checking (no-op for repos that don't need it)
    prepare_repo "$repo" "$repo_path"
...
$CHECK_OUTPUT"
            ...
            if [ $ERROR_COUNT -gt 0 ]; then
                ...
                # Exit with code 2 to provide feedback to Claude for automatic processing
                exit 2
            fi
        fi
        ;;
