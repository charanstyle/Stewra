...
| `fleet-access-requests` / `fleet-access-request` / `fleet-access-approve` / `fleet-access-deny` | `src/app/fleet/FleetPage.tsx` | the "Waiting on you" card; one request `<li>`; its two decision `<button>`s | Someone outside the org, running Stewra Bridge on the same physical computer as one of its machines, asking to see that machine. The card renders only while a request is `pending`; the buttons render only for an admin. Approving grants sight of THAT ONE MACHINE — not membership, not the rest of the fleet, not the ability to start a session. |
